...
try {
  ...
  events.map(async (eve) => {
    ...
    try {
      const { wallet, eventName, eventHash, lastBlockNumber, contractAddress } =
        eve;
      ...
      let messageBody = `Event ${eventName} was emited on ${new Date(
        Number(response.result[0].timeStamp) * 1000
      )}`;
